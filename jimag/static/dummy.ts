...
(function() {
	...
	const [selected, setSelected] = React.useState<{ chainid: string }[]>([]);
	...
	React.useEffect(() => {
		...
		plugin = await createPluginUI({ target: parent, spec: MySpec, render: renderReact18 });
	...
	for (const item of pocket.seq) {
		...
		const component = await plugin.builders.structure.tryCreateComponentFromExpression(currentStructure, query, `pocket${pocket.rank}`);
		if (!component) {
		console.warn(`Failed to create component for pocket ${pocket.rank}, chain ${chain}`);
		continue;
		}
