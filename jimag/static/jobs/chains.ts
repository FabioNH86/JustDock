...
import { StateObject, StateObjectRef, StateObjectSelector, StateTransformer } from 'molstar/lib/mol-state';
...
let loadedReprs: StateObjectSelector<SO.Molecule.Structure.Representation3D, StateTransformer<StateObject<any, StateObject.Type<any>>, StateObject<any, StateObject.Type<any>>, any>>[][] = [];
let components: { [key: string]: any } = {};

// Extend the Window interface to include custom properties
declare global {
	interface Window {
		deletePockets?: () => Promise<void>;
		loadReceptor?: (receptor: any) => Promise<void>;
		togglePocketRepr?: (pocket: Pocket, isChecked: boolean) => Promise<void>;
		clearChainString?: () => void;
		clearPocketsTable?: () => void;
	}
}
...
window.loadReceptor = async (receptor) => {
    loadedReprs = [];
    components = {};
	if (window.clearChainString) {
		window.clearChainString();
	}
	if (window.clearPocketsTable) {
...
async function pause(ms: number | undefined) {
